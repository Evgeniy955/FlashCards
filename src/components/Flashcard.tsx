...
export const Flashcard: React.FC<FlashcardProps> = ({ 
  ...
  // Ref to store the initial touch target, for distinguishing scroll vs swipe
  ...
  useEffect(() => {
    ...
    touchStartTargetRef.current = null; // Clear ref on word change
  }, [word]);
  ...
  const handleTouchStart = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    if (target.closest('button') || target.closest('select')) {
      // Allow default behavior for these elements (e.g., button click)
      return;
    }
  ...
  const handleTouchEnd = () => {
    ...
    setDragStart(null);
    setDragOffset({ x: 0, y: 0 });
    touchStartTargetRef.current = null; // Clear the ref
  };

  const handleFlipDuringChange = (e: React.MouseEvent) => {
    if (showSettings) return;
    // Prevent flip if currently changing word or if a drag/swipe gesture was just performed
