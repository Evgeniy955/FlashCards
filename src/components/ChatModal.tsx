...
export const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, userName }) => {
  ...
  const isProcessingRef = useRef(false); // To prevent restarting mic while AI is thinking/speaking
  const silenceTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const accumulatedTranscriptRef = useRef(''); // Stores text if mic restarts mid-sentence
  const inputRef = useRef(''); // Syncs with input state for access inside closures
  ...
  const startRecognition = () => {
    ...
    recognition.continuous = true; // Keep keeping on until we say stop
    ...
    recognition.onend = () => {
        // If we are in Continuous mode AND NOT processing AI response, 
        // we treat a stop as an accidental browser timeout and restart.
        if (conversationMode === 'continuous' && !isProcessingRef.current && isChatActive && isOpen) {
            // Save what we had so far if it wasn't added to input yet
            if (inputRef.current.trim()) {
                accumulatedTranscriptRef.current += inputRef.current + ' ';
            }
            // Restart immediately
            try {
                recognition.start();
    ...
    recognition.onerror = (event: any) => {
        ...
        // Ignore 'no-speech' errors in continuous mode, just keep going
    };
    ...
    recognition.onresult = (event: any) => {
      ...
      // Update UI
      // We combine any accumulated text (from previous restarts) with current
      const fullText = (accumulatedTranscriptRef.current + interimTranscript).trim();
      setInput(fullText);

      // CONTINUOUS MODE LOGIC: Wait for 3 seconds silence
      if (conversationMode === 'continuous') {
          if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
          
          // Only set timer if we actually have text
          if (fullText) {
              silenceTimerRef.current = setTimeout(() => {
                  // Silence detected!
                  isProcessingRef.current = true; // Stop auto-restart logic
                  recognition.stop(); // Stop mic
                  handleSendMessage(); // Send what we have
  ...
  const handlePTTStart = (e: React.SyntheticEvent) => {
      ...
      stopAllAudio(); // Clear any previous state
      setInput('');
      isProcessingRef.current = false;
      startRecognition();
  };
  ...
  const handlePTTEnd = (e: React.SyntheticEvent) => {
      ...
      // Stop logic
      isProcessingRef.current = true; // Mark as processing so onend doesn't restart
      if (recognitionRef.current) {
          recognitionRef.current.stop();
      }

      // Small delay to let final transcript process
      setTimeout(() => {
          if (inputRef.current.trim()) {
              handleSendMessage();
          }
  ...
  const speakText = (text: string) => {
    if ('speechSynthesis' in window) {
      ...
      // AUTO-RESUME for Continuous Mode
      utterance.onend = () => {
  ...
  const handleSendMessage = async (isInitial = false) => {
    ...
    // Prevent sending empty messages (unless initial)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Conversation Practice">
            ...
            <div className="flex flex-col border-b border-slate-200 dark:border-slate-700 pb-2 mb-2">
                ...
                <div className="flex items-center gap-4 bg-slate-100 dark:bg-slate-800 p-2 rounded-lg self-start">
                     <label className="flex items-center gap-1.5 cursor-pointer text-xs">
                         ...
                         />
                         <span className="text-slate-700 dark:text-slate-300">Push to Talk</span>
                     </label>
                     <label className="flex items-center gap-1.5 cursor-pointer text-xs">
                         <input 
                            type="radio" 
            ...
            <div className="flex items-center gap-2 mt-auto">
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex-1 flex gap-2">
                    <input
                        ...
                        disabled={isRecording && conversationMode === 'continuous'} // Disable manual typing in continuous mode to avoid conflict
                    />
                    <button
                        type="submit"
                        disabled={!input.trim() || isLoading || isRecording}
                        className="p-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full transition-colors disabled:opacity-50"
                    >
                        <Send size={20} />
                    </button>
                </form>
